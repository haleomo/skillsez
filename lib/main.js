...
async function saveLearningPlan(event) {
    ...
    console.log('[SavePlan] Current user:', currentUser);
    
    if (!currentUser || !currentUser.userId) {
        console.log('[SavePlan] No user or user ID found');
        showErrorAlert('User Not Found', 'Please register to save learning plans.');
        return;
    }
    ...
    try {
        ...
        console.log('[SavePlan] Button disabled, starting save...');

        // Step 1: Save the query profile
        console.log('[SavePlan] Saving query profile...');
        const profileResponse = await fetch(`${API_URL}/query-profile`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
        ...
        if (!profileResponse.ok) {
            ...
            throw new Error(`Failed to save query profile: ${profileResponse.status} - ${errorText}`);
        }

        const profileResult = await profileResponse.json();
        const queryProfileId = profileResult.data.id;
        console.log('[SavePlan] Query profile saved with ID:', queryProfileId);

        // Step 2: Save the learning plan (query result)
        console.log('[SavePlan] Saving learning plan...');
        const resultResponse = await fetch(`${API_URL}/query-result`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                queryProfileId: queryProfileId,
                queryResultNickname: `${queryProfile.topic} - ${new Date().toLocaleDateString()}`,
        ...
        showSuccessAlert('Plan Saved!', `Your learning plan for "${queryProfile.topic}" has been saved successfully.`);
